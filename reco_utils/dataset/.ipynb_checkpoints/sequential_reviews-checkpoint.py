...
if 'kwai' not in socket.gethostname():
    try:
        ...
        print("libKMCUDA not available, falling back to CPU-based k-means.")
...
def get_categories_by_clustering(meta_file, num_centroids, items):
    ...
    if kmeans_available:
        # Use kmeans_cuda
        _, assignments = kmeans_cuda(item_embed, num_centroids, verbosity=verbosity, seed=seed)
        return assignments
    else:
        # Use sklearn KMeans
        kmeans = KMeans(n_clusters=num_centroids, random_state=seed, verbose=verbosity)
        kmeans.fit(item_embed)
