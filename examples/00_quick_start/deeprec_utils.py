...
def check_nn_config(f_config):
    ...
    elif f_config["model_type"] in ["dance", "DANCE"]:
        required_parameters = [
            "item_embedding_dim",
            "cate_embedding_dim",
            "max_seq_length",
...
def create_hparams(flags):
    ...
    return tf.contrib.training.HParams(
        ...
        PRETRAIN_DIR=flags["PRETRAIN_DIR"] if "PRETRAIN_DIR" in flags else None,
        FINETUNE_DIR=flags["FINETUNE_DIR"] if "FINETUNE_DIR" in flags else None,
        # dkn
        ...
        pretrain_epochs=flags["pretrain_epochs"] if "pretrain_epochs" in flags else 20,
        finetune_epochs=flags["finetune_epochs"] if "finetune_epochs" in flags else 100,
        ...
        visual_step=flags["visual_step"] if "visual_step" in flags else 1,
        visual_type=flags["visual_type"] if "visual_type" in flags else None,
        ...
        # add for gcn
        train_dir=flags["train_dir"] if "train_dir" in flags else None,
        graph_dir=flags["graph_dir"] if "graph_dir" in flags else None,
        test_dropout=flags["test_dropout"] if "test_dropout" in flags else True,
        use_complex_attention=flags["use_complex_attention"] if "use_complex_attention" in flags else False,
        use_time4lstm=flags["use_time4lstm"] if "use_time4lstm" in flags else True,
